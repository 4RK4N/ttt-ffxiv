...
import { ensureCsrfToken, getAuthorizedSessionUser, handleCallback, logout, requireAuth, requireCsrf, startLogin, verifyFormCsrf, } from "./auth.js";
import { loadWebPlugins } from "./plugins.js";
...
export async function startWeb() {
    ...
    function resolveCssFile(file) {
        const local = resolve(cssDir, file);
        if (local.startsWith(resolve(cssDir)) && existsSync(local))
            return local;
        return null;
    }
    function resolveJsFile(file) {
        const local = resolve(jsDir, file);
        if (local.startsWith(resolve(jsDir)) && existsSync(local))
            return local;
        return null;
    }
    ...
    app.get("/callback", async (c) => {
        ...
        if (result.ok)
            return c.redirect("/");
        return c.html(loginPage(cfg.botName, result.message), result.status);
    });
    app.get("/logout", (c) => {
        logout(c);
        return c.redirect("/");
