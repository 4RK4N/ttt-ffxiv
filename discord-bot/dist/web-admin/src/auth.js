...
import { discordBotGetOptional, fetchGuildOwnerId, fetchGuildRolesRaw, } from "./discordCache.js";
...
export async function startLogin(c, cfg) {
...
async function isGuildAdmin(accessToken, guildId) {
 ...
 */
async function isGuildAdminViaBot(cfg, userId) {
    const member = await discordBotGetOptional(cfg, `/guilds/${cfg.guildId}/members/${userId}`);
    if (!member)
        return false;
    ...
    return roles.some((role) => typeof role.id === "string" &&
 ...
 */
async function isStillAdmin(cfg, userId, freshCheck = false) {
    if (!freshCheck) {
 ...
 */
export async function handleCallback(c, cfg) {
    ...
    await setSignedCookie(c, SESSION_COOKIE, JSON.stringify(user), cfg.sessionSecret, cookieOptions(cfg, SESSION_MAX_AGE));
...
export async function getSessionUser(c, cfg) {
 ...
 */
export async function getAuthorizedSessionUser(c, cfg) {
...
export async function getCsrfToken(c, cfg) {
...
export async function ensureCsrfToken(c, cfg) {
...
export async function verifyFormCsrf(c, cfg, formToken) {
...
export function requireCsrf(cfg) {
    return async (c, next) => {
