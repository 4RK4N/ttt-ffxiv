...
import { DataReadError, readValues, ValidationError, writeEnabled, writeValues, } from "../../store.js";
...
import { defaultObjectItem, defaultOptionItem, findObjectListField, getObjectListItems, getOptionListItems, mergeRowFromForm, rowKeyForItem, toggleExpanded, } from "./htmx-handlers.js";
...
async function ctxAndMod(c, deps, namespace) {
...
function panelProps(mod, ctx, expanded, status) {
    return { mod, ctx, expanded, status };
}
function publishItemIdError(itemId) {
    try {
...
export async function renderPanel(c, deps, namespace, expanded, status) {
    const data = await ctxAndMod(c, deps, namespace);
    if (!data)
        return c.text(`Unknown module "${namespace}".`, 404);
    return c.html(_jsx(ModulePanel, { ...panelProps(data.mod, data.ctx, expanded, status) }));
}
export function registerHtmxRoutes(htmx, deps) {
    ...
    htmx.put("/modules/:namespace", async (c) => {
        ...
        try {
            ...
            const mod = buildEditorModule(plugin);
            mod.values = saved;
            return c.html(_jsx(ModulePanel, { ...panelProps(mod, ctx, expanded, { ok: true, message: "Saved" }) }));
        }
        catch (err) {
            const message = err instanceof ValidationError || err instanceof DataReadError
                ? err.message
            ...
            const mod = buildEditorModule(plugin);
            mod.values = values;
            return c.html(_jsx(ModulePanel, { ...panelProps(mod, ctx, expanded, { ok: false, message }) }), 400);
        }
    });
    htmx.put("/modules/:namespace/enabled", async (c) => {
        ...
        try {
            ...
            console.log(`[web] ${c.get("user").username} ${enabled ? "enabled" : "disabled"} module "${namespace}".`);
            return c.html(_jsx(EnabledToggleResponse, { namespace: namespace, enabled: enabled }));
        }
        catch (err) {
    ...
    htmx.post("/modules/:namespace/list/:fieldKey/add", async (c) => {
        ...
        const csrfToken = await ensureCsrfToken(c, deps.cfg);
        const ctx = await loadEditorContext(deps.cfg, csrfToken);
        return c.html(_jsx(ObjectListRowsOnly, { field: field, items: items, ctx: ctx, namespace: namespace, expanded: expanded }));
    });
    ...
    htmx.post("/modules/:namespace/row/:fieldKey/:index/toggle", async (c) => {
        ...
        const csrfToken = await ensureCsrfToken(c, deps.cfg);
        const ctx = await loadEditorContext(deps.cfg, csrfToken);
        return c.html(_jsx(ObjectListRow, { field: field, row: row ?? {}, rowIndex: index, ctx: ctx, namespace: namespace, expanded: expanded }));
    });
    ...
    htmx.post("/modules/:namespace/list/:fieldKey/option/:optionKey/add/:rowIndex", async (c) => {
        ...
        return c.html(_jsx(OptionListRowsOnly, { f: sub, name: name, items: opts, ctx: ctx, namespace: namespace, fieldKey: fieldKey, rowIndex: rowIndex }));
    });
    htmx.post("/modules/:namespace/list/:fieldKey/option/:optionKey/remove/:rowIndex/:optIndex", async (c) => {
        const namespace = c.req.param("namespace");
        const fieldKey = c.req.param("fieldKey");
        const optionKey = c.req.param("optionKey");
        const rowIndex = Number(c.req.param("rowIndex"));
    ...
    htmx.post("/modules/:namespace/publish/:itemId", async (c) => {
        ...
        try {
            ...
            return c.html(_jsx(ModulePanel, { ...panelProps(mod, ctx, expanded, {
                    ok: true,
                    message: "Published",
                }) }));
        }
