...
import { isReactionType, normalizeRoleOptions, } from "./types.js";
const MAX_OPTIONS = MAX_PANEL_OPTIONS;
export function validateRolePanel(panel) {
    if (!isReactionType(panel.reactionType)) {
        throw new Error(`Invalid reaction type "${String(panel.reactionType)}". Must be one of: button, emoji, dropdown, dropdown-single.`);
    }
    ...
    if (panel.reactionType === "emoji") {
        ...
        for (const opt of panel.roleOptions) {
            ...
            if (key && seenEmoji.has(key)) {
                throw new Error(`Duplicate emoji "${opt.emoji.trim()}" — each option must use a different emoji in emoji reaction mode.`);
            }
            if (key)
                seenEmoji.add(key);
        }
...
export function validateRolePanelRow(configRow, textRow) {
    const rawType = configRow.reactionType;
    const reactionType = isReactionType(rawType)
        ? rawType
        : "button";
    if (rawType !== undefined && rawType !== null && !isReactionType(rawType)) {
        throw new Error(`Invalid reaction type "${String(rawType)}". Must be one of: button, emoji, dropdown, dropdown-single.`);
    }
    ...
    const panel = {
        ...
        ephemeralMessage: typeof textRow.ephemeralMessage === "string"
