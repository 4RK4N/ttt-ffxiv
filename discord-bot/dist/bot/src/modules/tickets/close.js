...
import { buildConfirmRow, CLOSE_CONFIRM_PREFIX, } from "../../lib/modules/tickets/panel.js";
...
export async function handleCloseTicket(interaction) {
    ...
    const flow = await runTicketConfirmFlow({
        ...
        buildConfirmRow,
        canPerform: () => canCloseTicket(interaction, openerUserId, ticketType.staffRoleId),
    ...
    try {
        await finalizeTicketClose(thread, parsed.typeId, ticketType, ticketType.ticketClosed, welcomeMessage, openerUserId);
    }
    catch (err) {
