...
import { data, NAMESPACE, resolveDeleteAuthorLastMention, resolveDeleteEmoji, } from "../../lib/modules/pic-repost-commands/config-io.js";
...
async function handleDeleteReaction(reaction, user, botUserId) {
    if (!botUserId)
        return;
    ...
    const cfg = data();
    if (!reactionsMatch(resolveDeleteEmoji(cfg), reaction.emoji.name, reaction.emoji.id ?? null)) {
        return;
    }
    const authorUserId = resolvePicRepostAuthor(message, botUserId, resolveDeleteAuthorLastMention(cfg));
    if (!authorUserId)
        return;
    if (user.id !== authorUserId) {
        try {
            ...
            console.warn(`[${NAMESPACE}] Failed to remove non-author delete reaction message=${message.id} user=${user.id}:`, err);
        }
        return;
    }
    try {
        ...
        if (isDiscordUnknownMessage(err))
            return;
        console.error(`[${NAMESPACE}] Failed to delete post message=${message.id}:`, err);
    }
}
export function registerDeleteReactionHandler(client) {
    registerSafeHandler(client, Events.MessageReactionAdd, (reaction, user) => handleDeleteReaction(reaction, user, client.user?.id), `[${NAMESPACE}]`);
}
