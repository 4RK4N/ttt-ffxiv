...
async function syncEmojiReactions(ctx, channelId, messageId, options) {
...
export async function publishPanel(ctx, panelId, channelId, existingMessageId) {
    ...
    const afterPublish = panel.reactionType === "emoji"
        ? (messageId) => syncEmojiReactions(ctx, channelId, messageId, panel.roleOptions)
        : undefined;
...
export function matchOptionByReaction(options, emojiName, emojiId) {
    for (const opt of options) {
        if (reactionsMatch(opt.emoji, emojiName, emojiId))
            return opt;
    }
    return undefined;
}
