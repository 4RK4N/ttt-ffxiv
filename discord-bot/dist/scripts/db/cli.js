...
import { closeDb, getDb, initDb, loadDbBootstrapConfig, } from "#shared/core/db.js";
import { setDbDataMany } from "#shared/core/dbData.js";
import { APP_CONFIG_TABLE, MODULE_NAMESPACES, moduleTableName, } from "#shared/core/moduleTable.js";
...
function buildAppConfigFromEnv() {
    ...
    const config = {
        ...
        sessionSecret: process.env.TTT_SESSION_SECRET?.trim() || randomBytes(32).toString("hex"),
...
async function writeAppConfigFromEnv() {
...
async function applySqlFile(dbPath, sqlFile) {
...
async function countAppConfig(dbPath) {
    await initDb({ dbPath: path.resolve(dbPath) });
    try {
        const stmt = await getDb().prepare(`SELECT COUNT(*) AS count FROM ${APP_CONFIG_TABLE}`);
...
async function dumpDb(dbPath, includeSecrets) {
    ...
    try {
        const schemaStmt = await db.prepare(`SELECT name, sql FROM sqlite_master WHERE type = 'table' AND name NOT LIKE 'sqlite_%' ORDER BY name`);
        const tables = (await schemaStmt.all());
        const lines = [
        ...
        for (const { name, sql } of tables) {
            ...
            const rowStmt = await db.prepare(`SELECT key, value FROM ${name} ORDER BY key`);
            const rows = (await rowStmt.all());
            for (const row of rows) {
                ...
                lines.push(`INSERT INTO ${name}(key,value) VALUES(${sqlStringLiteral(row.key)},${sqlStringLiteral(value)}) ` +
